...
class ManufacturingProductMediaTab extends StatelessWidget {
  const ManufacturingProductMediaTab(
              ...
              onPressed: () async {
                ...
                if (response is XFile?) {
                  BlocProvider.of<ProductBloc>(context).add(
                    PostNewMediaEvent(
                        photo: response, productId: manufacturingProduct.id!),
                  );
                }
              },
            ),
            ...
            builder: (context, state) {
              if (state is ProductsLoaded) {
                List<Map<String, dynamic>> productMedias = state.products
                    .firstWhere((p) => p.id == manufacturingProduct.id)
                    .medias!;
                return SizedBox(
                  height: height * 0.47,
                  child: Padding(
